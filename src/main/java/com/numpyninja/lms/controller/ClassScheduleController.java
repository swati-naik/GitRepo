...
public class ClassScheduleController {
	...
	@Autowired
	ClassManagementService classServices;

	//createClass
	@PostMapping(path="/CreateClassSchedule",consumes = "application/json", produces = "application/json")  
	@ResponseBody
	private ResponseEntity<?> createAndSaveClass(@Valid @RequestBody ClassScheduleDto classDTO)throws  DuplicateResourceFound
	{  
		System.out.println("in create a new class schedule");
		ClassScheduleDto savedClassDTO = classServices.createClass(classDTO);
	...
	{ 
		...
		List<ClassScheduleDto> ClassesList = classServices.getAllClasses();
	...
	{  
		...
		ClassScheduleDto classesDTOList= classServices.getClassByClassId(classId);
	...
	//GetAllClassesByClassTopic
		...
		{ 
			...
			List<ClassScheduleDto> ClassesList = classServices.getClassesByClassTopic(classTopic);
			return ResponseEntity.ok(ClassesList);  
		} 
		...
		{  
			...
			List<ClassScheduleDto> classesDTOList= classServices.getClassesByBatchId(batchId);
			return ResponseEntity.ok(classesDTOList);
		}  
		...
		{  
			...
			List<ClassScheduleDto> classesDTOList= classServices.getClassesByStaffId(staffId);
			return ResponseEntity.ok(classesDTOList);
		}  
	...
	@ResponseBody
	private ResponseEntity <ClassScheduleDto> updateClassScheduleById(@PathVariable @NotBlank @Positive Long classId ,@Valid @RequestBody ClassScheduleDto modifyClassSchedule) throws ResourceNotFoundException
	{  
